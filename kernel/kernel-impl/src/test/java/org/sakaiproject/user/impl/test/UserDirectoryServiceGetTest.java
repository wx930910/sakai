...
public class UserDirectoryServiceGetTest extends SakaiKernelTestBase {
	public static UserDirectoryProvider mockUserDirectoryProvider1() {
		UserDirectoryProvider mockInstance = mock(UserDirectoryProvider.class);
		doAnswer((stubInvo) -> {
			Collection users = stubInvo.getArgument(0);
			for (Iterator<User> iter = users.iterator(); iter.hasNext();) {
				UserEdit userEdit = (UserEdit) iter.next();
				if (!mockInstance.getUser(userEdit)) {
					iter.remove();
				}
			}
			return null;
		}).when(mockInstance).getUsers(any());
		when(mockInstance.getUser(any(UserEdit.class))).thenAnswer((stubInvo) -> {
			UserEdit userEdit = stubInvo.getArgument(0);
		...
		when(mockInstance.findUserByEmail(any(UserEdit.class), any(String.class))).thenAnswer((stubInvo) -> {
			UserEdit userEdit = stubInvo.getArgument(0);
			String email = stubInvo.getArgument(1);
		...
		when(mockInstance.authenticateUser(any(String.class), any(UserEdit.class), any(String.class)))
				.thenAnswer((stubInvo) -> {
					String eid = stubInvo.getArgument(0);
					UserEdit userEdit = stubInvo.getArgument(1);
		...
		when(mockInstance.authenticateWithProviderFirst(any(String.class))).thenAnswer((stubInvo) -> {
			String eid = stubInvo.getArgument(0);
	...
	private static UserDirectoryProvider userDirectoryProvider;
	private UserDirectoryService userDirectoryService;

	/**
	 * Because framework service calls create so many irreversible side-effects, we
	 * don't want to keep a single instance of the component manager alive for all
	 * test classes. On the other hand, we'd rather not take the overhead of
	 * starting up the component manager for every method in the test class. As a
	 * compromise, we run a test suite consisting only of the class itself.
	 * 
	 * @throws Exception
	 */
	@BeforeClass
	...
	private static void oneTimeSetupAfter() throws Exception {
		userDirectoryProvider = UserDirectoryServiceGetTest.mockUserDirectoryProvider1();
		...
		// Sakai user services very helpfully lowercase input EIDs rather than leaving
		// them alone.
	...
	private static User addUserWithEid(UserDirectoryService userDirectoryService, String eid) throws Exception {
		...
		User user = userDirectoryService.addUser(null, eid, "J. " + eid, "de " + eid, eid + "@somewhere.edu",
				eid + "pwd", "Guest", props);
	...
	private static boolean isAsExpected(User user, String eid, boolean isLocal) {
		...
		boolean isMatch = (eid.equals(user.getEid()) && user.getId().equals(eidToId.get(eid))
				&& user.getEmail().equals(eid + "@somewhere.edu") && user.getFirstName().equals("J. " + eid)
				&& user.getLastName().equals("de " + eid)
				&& ((isLocal && user.getType().equals("Guest")
						&& "local".equals(user.getProperties().get(USER_SOURCE_PROPERTY)))
						|| (!isLocal && user.getType().equals("Student")
								&& "provided".equals(user.getProperties().get(USER_SOURCE_PROPERTY)))));
		return isMatch;
	}
	...
	public void testLocalUserEidEdit() throws Exception {
		...
		// Currently there's no way for clients to directly find out whether separate
		// IDs and EIDs are
		// supported, and so we guess at it by looking at what ID was given to an
		// existing record.
...
//			if (user != null) log.debug("After removeUser, user eid=" + user.getEid() + ", id=" + user.getId());
	...
	public void testProvidedAuthentication() throws Exception {
		...
		user = userDirectoryService.authenticate("localwithfailedproviderauthn",
				"localwithfailedproviderauthn" + "pwd");
...
//		log.debug("Return from getUsers=" + users);
	...
	public void testNoDuplicateUserEid() throws Exception {
		...
		try {
			userDirectoryService.addUser(null, eid, "J. " + eid, "de " + eid, eid + "@somewhere.edu", eid + "pwd",
					"Guest", null);
			Assert.fail();
		} catch (Exception e) {
		}
	...
	/**
	 * We should get a predicatable error on trying to create a user with an Eid
