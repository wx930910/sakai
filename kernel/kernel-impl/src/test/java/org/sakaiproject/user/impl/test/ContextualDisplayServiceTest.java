...
/**
 * Test that the User Directory Service is correctly enabling context-specific
 * display IDs and display names.
 */
@Slf4j
public class ContextualDisplayServiceTest extends SakaiKernelTestBase {
	public static ContextualUserDisplayService mockContextualUserDisplayService1() {
		ContextualUserDisplayService mockInstance = mock(ContextualUserDisplayService.class);
		when(mockInstance.getUserDisplayId(any(User.class))).thenAnswer((stubInvo) -> {
			User user = stubInvo.getArgument(0);
			return mockInstance.getUserDisplayId(user, currentSiteUid);
		});
		when(mockInstance.getUserDisplayName(any(User.class), any(String.class))).thenAnswer((stubInvo) -> {
			User user = stubInvo.getArgument(0);
			String contextReference = stubInvo.getArgument(1);
		...
		when(mockInstance.getUserDisplayName(any(User.class))).thenAnswer((stubInvo) -> {
			User user = stubInvo.getArgument(0);
			return mockInstance.getUserDisplayName(user, currentSiteUid);
		});
		when(mockInstance.getUserDisplayId(any(User.class), any(String.class))).thenAnswer((stubInvo) -> {
			User user = stubInvo.getArgument(0);
			String contextReference = stubInvo.getArgument(1);
			if (contextualSiteUid.equals(contextReference)) {
				if (!NOT_IN_SITE_USER_EID.equals(user.getEid())) {
...
//			log.warn(e.getMessage(), e);
	...
	// to be handled less realistically within a single classloader and by explicit
	// injection
	// through Java code.
	...
	private static void oneTimeSetupAfter() throws Exception {
		...
		contextualUserDisplayService = ContextualDisplayServiceTest.mockContextualUserDisplayService1();
	...
	private void checkForAdvisedProperties(User user) {
		...
		Assert.assertEquals(user.getFirstName() + " " + user.getLastName() + DISPLAY_ADVISED_SUFFIX,
				user.getDisplayName());
	}
	...
	private void checkExplicitContextualService(User user) {
		...
		Assert.assertEquals(user.getEid() + CONTEXTUAL_SITE_NAME,
	...
	private User addUserWithEid(String eid) throws Exception {
		User user = userDirectoryService.addUser(null, eid, FIRST_NAME, eid, eid + "@somewhere.edu", eid + "pwd",
				"Guest", null);
//		log.debug("addUser eid=" + eid + ", id=" + user.getId());
	...
	public static class TestDisplayAdvisorProvider implements UserDirectoryProvider, DisplayAdvisorUDP {
		...
		// This test class only acts as a display name advisor, not as a user record
		// provider.
	...
	/**
	 * Convenience method to switch authn/authz identities. TODO Move this
	 * frequently-needed helper logic into the base class.
	 *
	 * @param userEid
	 */
