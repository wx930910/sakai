...
public class CategoryDefinition implements Serializable {
    ...
    public Double getTotalPoints() {
        ...
        if (assignmentList != null) {
            return assignmentList.stream().reduce(
                BigDecimal.ZERO, (a, b) -> a.add(BigDecimal.valueOf(b.getPoints())), (a, b) -> a.add(b)).doubleValue();
        } else {
            return 0D;
        }
    }
    ...
    public boolean isAssignmentInThisCategory(String assignmentId) {
        ...
        return assignmentList.stream().filter(a -> a.getExternalId() != null)
            .anyMatch(a -> a.getExternalId().equalsIgnoreCase(assignmentId));
    }
